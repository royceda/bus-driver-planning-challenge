...
int z[1..n][1..m] = ...;// le chauffeur i travail dans la ligne j
 ...
 subject to{
  	forall(i in 1..n, j in 1..T){
  	 	ct1 : x[i][j] + y[i][j] <= 1; //matin, aprem ou congés
  	 	ct41: x[i][j] + d[i][j] <= 1; //congés matin
   		ct42: y[i][j] + d[i][j] <= 1; //congés aprem
   }  	 	
   ...
   forall(j in 1..T, i in 1..n){
  	...
  	//forall(j in 1..T){
		//ct61: sum(i in 1..n) y[i][j] >= 3; // service assuré chaque jour pour les 3 lignes	
		//ct62: sum(i in 1..n) x[i][j] >= 3; // service assuré chaque jour pour les 3 lignes	
	//}
